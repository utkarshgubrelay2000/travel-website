...
//console.log(req.body)
  category
    .findOneAndUpdate(
      { _id: categoryId },
      { categoryName: categoryName, thumbImage: thumbImage }
    )
    .then((changedCategory) => {console.log(changedCategory)
      res.json({ message: "Category Updated",changedCategory:changedCategory });
    })
    .catch((err) => {
      res.status(503).json("something went wrong");
    });
